...
package com.example.automodifylog.uitils.pinyin;
    ...
    public static String getPinYinHeadCharToUpperCase(String str) {
       StringBuffer newWord = new StringBuffer();
        for (int j = 0; j < str.length(); j++) {
            ...
            if (pinyinArray != null) {
                // + ""转换为 String
                chr = pinyinArray[0].charAt(0)+"";
            } else {
                chr = word+"";
            }
            convert.append(chr.toUpperCase());
            convert.append(pinyinArray[0].length() > 1 ? pinyinArray[0].substring(1,pinyinArray[0].length()-1) : null);
            newWord.append(convert);
        }
        return newWord.toString();
    }

    /**
     * 获取全拼并首字母大写 eg: eg: 中国--》ZG
     * */
    public static String getHeadCharToUpperCase(String str) {
        StringBuilder convert = new StringBuilder();
        for (int j = 0; j < str.length(); j++) {
            char word = str.charAt(j);
            String[] pinyinArray = PinyinHelper.toHanyuPinyinStringArray(word);
